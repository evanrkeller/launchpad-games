...
def limit(n, minVal, maxVal): return max(min(maxVal, n), minVal)
def limit1(n): return limit(n, 0.0, 1.0)
...
class Fire:
    ...
    def __init__(self, xSize=8, ySize=8):
        ...
        self.sizeX = xSize
        self.sizeY = ySize
        # becomes a 2D list of floats (yep, no array.array due to fck mth)
        self.matrix = None
    ...
    def MatrixFill(self, value=None):
        ...
        """
        if value is None:
            self.matrix = [[random.randint(
                0, 10)/10.0 for i in range(self.sizeX)] for y in range(self.sizeY)]
        else:
            self.matrix = [[limit1(float(value))] *
                           self.sizeX for y in range(self.sizeY)]
    ...
    def MatrixEvolveAtXY(self, x, y):
        ...
        valBelow = sum(self.MatrixGet3ValBelowXY(x, y)) / \
            3.0 - 0.07 - (0.02*random.random())
...
# if buts[0:2] == [ 0, 8 ] and not buts[2]:
            ...
            if 0 < buts[0] < 9 and 0 < buts[1] < 9 and not buts[2]:
                print(
                    "Don't push the matrix buttons. Might create a lag (pressure events).")
            # --- quit
# elif buts[0:2] == [ 0, 1 ]:
            elif buts[0:2] == [8, 1]:
                break
